...
**작품·회차 관리** 에서 작품 이름을 눌러 펼치면:

- **✏️ 작품 정보 수정** — 제목, 장르, 상태, 소개, **대표 작품 지정(⭐)**, **인기도(🔥)**, 정렬 순서를 바꾸고 **정보 저장**을 누르면 됩니다.
  - **대표 작품** 체크 → 메인 상단 "⭐ 대표 작품"에 노출
  - **인기도** 를 1 이상으로 → 메인 "🔥 인기 작품"에 노출 (숫자가 클수록 위로)
- **회차별 삭제**, 작품 **전체 삭제**(표지 포함)도 여기서 합니다.
- **표지만 바꾸기** → 위 "회차 업로드"에서 그 작품을 선택하고 표지 이미지만 올린 뒤(원고 없이) 업로드하면 표지가 교체됩니다.

"목록 새로고침" 으로 최신 상태를 다시 불러옵니다.
...
**Q. 상단 문피아 배너의 링크/문구를 바꾸려면?**
A. `app/site.config.json` 의 `promoBanner` 를 고치면 됩니다. `enabled` 를 `false` 로 하면 배너가 사라집니다.
```json
"promoBanner": { "enabled": true, "text": "📚 문피아 작가의 유료/신규 소설 보러가기", "url": "https://..." }
```

**Q. 구글 애드센스는 어떻게 넣나요? (승인 전/후)**
A. `app/site.config.json` 의 `adsense` 에 이미 `client`/`slot` 이 들어가 있고 `enabled: true` 입니다. 이 상태로 배포하면 **애드센스 로더 스크립트가 사이트에 실려서 구글 심사(승인)에 필요한 조건이 충족**됩니다. 승인 전에는 광고 자리가 **빈칸**으로 있다가, **"준비됨"(승인)되면 자동으로 광고가 채워집니다.** 그러니 지금 넣어두는 게 맞습니다.
- 광고 위치: 메인(중단·하단), 소설 읽기 페이지(본문 위·아래), 작품목록·작품상세(하단)에 자동으로 들어갑니다.
- 광고를 더 넣고 싶으면 원하는 위치의 HTML에 `<div class="ad-slot"></div>` 를 추가하면 그 자리에도 광고가 뜹니다.
- 잠깐 끄려면 `"adsense": { "enabled": false, ... }` 로 바꾸면 됩니다.
- 참고: 위치별로 다른 광고 단위를 쓰고 싶으면, 각 `ad-slot` 에 `data-ad-slot="다른슬롯번호"` 를 넣으면 그 슬롯이 우선 적용됩니다.

**Q. 작품별 SEO 페이지(검색 노출용)는 어떻게 되나요?**
A. 작품마다 검색엔진용 정적 페이지가 `/novel/{작품폴더}` 주소로 자동 생성됩니다. (예: `/novel/f-ss`) 제목·소개·회차 목록이 HTML에 직접 들어있어 구글·네이버가 잘 읽습니다. 관리자에서 업로드·정보수정·삭제하면 자동 갱신되고, 기존 작품 전체를 한 번에 만들려면 **관리자 → 작품·회차 관리 → "SEO 랜딩페이지 전체 생성"** 버튼을 누르면 됩니다. 홈의 작품 카드도 이 주소로 연결됩니다. sitemap.xml에도 자동 포함되니, 구글 서치콘솔·네이버 서치어드바이저에 사이트맵(`https://내주소/app/sitemap.xml`)을 제출하면 각 작품 페이지가 색인됩니다.

**Q. 정말 완전 무료인가요?**
A. 네. GitHub, Cloudflare Pages(Functions 포함) 모두 개인 웹소설 사이트 규모에선 무료 한도 안에서 운영됩니다.
