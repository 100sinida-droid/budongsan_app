...
> 동작 원리: 관리자 페이지에 **로그인 → 원고·표지를 드래그 업로드**하면, 백엔드가 GitHub에 한 번에 커밋 → GitHub Action이 목록·검색·사이트맵을 자동 생성 → Cloudflare Pages가 자동 재배포합니다. **로그인해서 올리기만 하면 홈페이지에 자동 반영됩니다.** (GitHub 토큰은 서버에만 있고 브라우저로 오지 않습니다.)
...
├── functions/
│   └── api/[[path]].js      ← 관리자 백엔드 (로그인·업로드·삭제 처리)
├── .github/workflows/
...
> 루트에는 진입 페이지(`index.html`, `admin.html`)와, 반드시 루트에 있어야 하는 시스템 폴더 두 개(`functions/` = 관리자 백엔드, `.github/` = 자동 인덱싱)만 있습니다. 그 외 모든 것은 **`app/`** 안에 있어 나중에 폴더째 백업·교체할 수 있습니다.
>
...
4. **업로드 실행** → 한 번의 커밋으로 올라갑니다. **1~2분 뒤** 자동 인덱싱·배포가 끝나면 사이트에 반영됩니다.
