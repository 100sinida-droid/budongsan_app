...
async function commitTree(CFG, entries, message) {
  ...
  // 트리 구성
  //  - 텍스트(utf-8): content 를 트리에 그대로 인라인 → 파일마다 별도 호출이 없어 무제한 업로드 가능
  //  - 이미지(base64): 인라인 불가라 blob 1개만 생성
  //  - 삭제: sha:null
  const tree = [];
  for (const e of entries) {
    if (e._blob) {
      if (e._blob.encoding === 'base64') {
        ...
        if (!br.ok) throw new Error('이미지 blob 생성 실패: ' + (await br.text()).slice(0, 150));
        tree.push({ path: e.path, mode: e.mode, type: 'blob', sha: (await br.json()).sha });
      } else {
        tree.push({ path: e.path, mode: e.mode, type: 'blob', content: e._blob.content });
      }
    } else {
